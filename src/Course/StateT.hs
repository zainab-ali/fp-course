...
distinct' ::
  ...
  -> List a
distinct' =
  (flip eval') S.empty . filtering (\a -> state' (S.notMember a &&& S.insert a))
...
distinctF ::
  ...
  -> Optional (List a)
distinctF =
  (flip evalT) S.empty . filtering (\a -> StateT (if a > 100
                                                 then const Empty
                                                 else Full . (S.notMember a &&& S.insert a)))
...
instance Monad f => Monad (OptionalT f) where
  f =<< OptionalT a = OptionalT ((optional (runOptionalT . f) (pure Empty)) =<< a)
...
distinctG =
  runOptionalT . (flip evalT) S.empty . filtering (\a -> StateT (\s -> OptionalT (
                            if a > 100
